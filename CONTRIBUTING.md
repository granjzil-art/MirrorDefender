...
- 网格坐标继续封装在 Grid 层；**正式产品只维护正方形（SQUARE）关卡，六边形（HEX）已经废弃**。既有 HEX 代码可暂时保留以避免无关清理，但后续功能不得为六边形增加适配、兼容、迁移或验收工作，除非用户重新明确扩大范围。
- **唯一正式关卡目录是 `res://resources/levels/`；该目录内全部关卡均需随数据契约持续迁移、校验和验收。目录外的关卡资源全部视为废弃，不是玩法兼容目标。** 自动化测试可以使用最小化 SQUARE 夹具，但测试夹具不获得正式关卡地位，也不得反向要求新功能兼容废弃内容。
- 数据与逻辑分离。
...
- **`@tool Resource` 导出字段迁移必须保护既有资源（硬规范）**：禁止在 Godot 编辑器仍打开相关资源时直接删除/重排 `@export` 字段并依赖热重载迁移；必须先保存并记录正式 `.tres` 参数，完成字段兼容/迁移后重新加载编辑器，再审查资源 diff，并用生产资源回归锁定关键值。不得把 Inspector/UndoRedo 自动写回造成的参数变化当作设计调整提交。
