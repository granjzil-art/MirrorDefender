...
> M5 起边屏障与复制镜通过 `EdgeOccupancyRegistry` 共用 `canonical_edge_id` 物理边占用；任一实体边建筑存在时，另一类放置和预览都会拒绝重叠。
...
| `scripts/ui/BuildingActionPanel.gd` | `BuildingActionPanel` / `Control` | 边建筑与其它塔一样只显示左降级/右升级，不显示说明、拆除或旋转选项。 |
...
运行 `scenes/Main.tscn`，在右上建筑面板选择“边障”。鼠标靠近任意内部共享边：合法时显示贴边蓝色双向虚影，左键建造。无需先制作敌人路径；选择已建边屏障后可从左侧降级或从右侧升级，建筑操作面板不显示说明、拆除或旋转按钮。
