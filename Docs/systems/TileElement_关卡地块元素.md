...
- 敌人逻辑上进入“当前格 → 下一阻碍格”路径段时安装临时路由；不要求单位世界坐标仍精确在格心。
- 候选只能通往敌人初始路径锁定的目标据点。先查找其他手工 PathDefinition 的完整可用后缀；必须包含当前格（接入代价 0）或有一格与当前格相邻（代价 1）。
...
- 无完整手工后缀时，在“所有同目标手工路径格”并集上执行确定性 A*，可串联相交/相邻的路径片段；不使用全地图可通行格。
