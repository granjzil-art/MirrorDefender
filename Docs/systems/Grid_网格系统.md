...
> **正式维护范围（2026-08-10 起）**：`res://resources/levels/` 内的正式关卡只使用正方形（SQUARE）；六边形（HEX）已经废弃。HEX 实现可暂时留在代码库，但新功能不得为其增加适配、兼容、迁移或验收；下文 HEX 内容仅说明历史代码，不构成开发要求。
...
- **边(Edge)** 是一等公民：正式 SQUARE 每格 4 条边，以 `(cell, edge_index)` 定位，以 `canonical_edge_id` 标识唯一物理边。`GridCellData.edge_building_mask` 分别存储右/上/左/下权限，编辑一条边时同步其相邻格的反向位；缺省四边全部允许。

- **多格对象边权限叠加**：3×2 据点不破坏底层 Grid 的作者权限，而由 TerrainManager 按占地派生有效权限：同一据点六格之间的内部边禁用，只有外轮廓边保留底层权限。移动/旋转/删除多格对象后会自动恢复 Grid 原权限，避免破坏性改写。
