...
## 职责
提供程序启动选关、局内 HUD 与操作入口，承载六槽分页、可建造卡槽、只读塔图鉴、资源、建筑/镜子说明、逐波释放、全局状态、时间控制和退关生命周期。M6 的整体布局、批次边界与验收事实源为 `Docs/07_M6_操作与UI大版本_需求与开发计划.md`。
...
- **运行时关卡元素编辑**：左侧“关卡元素编辑”开启 Stuff-only 作者工作区。进入后临时抑制可建造卡、镜卡和只读塔图鉴，冻结玩法时间但保留 UI/镜头输入；退出工作区后图鉴自动恢复，底部建筑卡仍保留其 F2 状态。目录按钮选择元素，左键放置或选择，`R` 旋转，“删除选中元素”按钮或 `Delete` 删除，`Ctrl+Z/Ctrl+Y` 撤销/重做，右键取消当前工具。删除按钮只在选中已有 Stuff 时启用，同格多实例逐个删除。真实模型预览以绿/红叠色表示可放置/非法或断路警告。
...
- **建筑/镜子卡悬停说明（2026-08-16 布局更新）**：悬停建筑卡、复制镜卡或反射镜卡时，`BuildCardBar` 显示无输入拦截的自适应说明框，移出即隐藏。首行显示对象名，后续使用镜子两行/塔三行语义标题与白色正文；`RichTextLabel` 使用 22/18 号标题/正文和紧凑行距。底部建筑卡仍在卡槽正上方显示；左上镜卡因上方空间不足，自动翻到卡槽下方并做视口钳制。只读塔图鉴使用独立说明框并固定向卡右侧展开。两者内容都与实体说明按钮共用同一 Definition BBCode 入口；程序镜面和完整卡面模式都支持，空槽不触发。
...
- **左上常驻调试信息（批次 6 已实现）**：控制台勾选或 `debug set` 启用的分类会同步显示在游戏画面左上区域；为避让常驻镜卡，摘要框左边界迁到 `x=240`。关闭控制台后继续按真实时间刷新，全部关闭时自动收起。
- **现役布局**：程序启动/退关时由 `LevelSelectView` 在纯黑幕上仅显示 2×3 六槽缩略图和两侧翻页箭头。局内左上常驻两张镜子卡，其下方左边侧纵向常驻四张只读塔图鉴；底部建筑卡默认关闭并由 F2 切换；右上是六项无框图标统计，最右侧中部是三波次按钮，右下只保留时间控制，暂停与控制台位于模态层。左侧不实例化地块详情或正式波次时间轴。
...
| BuildCardBar.`mirror_slot_separation` | 14 | 左上复制镜与反射镜两卡之间的像素间距。 |
| BuildCardBar.`mirror_cards_position` | `(18,68)` | 两张镜卡所在 `MirrorCards` 行的左上局部坐标。 |
| BuildCardBar.`building_left_margin/right_margin/bottom_margin` | `210 / 390 / 12` | 底部建筑卡在预留左右 HUD 区域之间居中，并保持底边距。 |
| RuntimeHud.`building_cards_visible_by_default` | false | 正式建筑卡组初始显隐；现役默认关闭，F2 运行时切换。 |
| TowerCodexPanel.`feature_enabled` | true | 左侧只读塔图鉴总开关；默认常驻，不接入 F2。 |
| TowerCodexPanel.`codex_position/card_size/card_separation` | `(18,218) / (84,92) / 8` | 四张图鉴卡的左侧起点、单卡尺寸与垂直间距。 |
| TowerCodexPanel.`description_width/description_gap` | `360 / 12` | 自适应说明的固定换行宽度和卡右侧间隔。 |
| RuntimeHud.`CardStyleToggle` | `(712,68)` | F1 调试工具开启时，在程序镜面和原画卡面之间即时切换；运行时元素编辑开启时随卡槽隐藏。 |
| RuntimeStuffEditorPanel.`debug_entry_position` | `(712,116)` | F1 调试入口组起点；避开左侧塔图鉴与左上镜卡，进入关卡时默认隐藏。 |
...
| `scripts/ui/RuntimeHud.gd` | `RuntimeHud` / `Control` | M6 正式 HUD 组合根；连接可建造卡、只读塔图鉴、全局/经济、时间、暂停/胜利/失败模态、破坏性操作确认、右侧波次三按钮和调试控制台。 |
...
| `tests/runtime_ui_batch4_test.gd` | 无 / `SceneTree` | 61 项只读波次模型、三按钮 18px 防误触间距、左上镜卡避让、共享据点生命、多路径流向和三档分辨率回归。 |
| `tests/runtime_ui_batch6_test.gd` | 无 class_name / `SceneTree` | 93 项 F1 调试工具、F2 建筑卡开关、生产四塔图鉴顺序/独立显隐/调试入口避让、注册表、八类开关、业务命令、控制台布局、常驻摘要、统一模态和旧入口迁移回归。 |
...
| `BuildCardBar.gd` | `set_building_cards_visible(visible: bool) -> void` / `toggle_building_cards() -> void` / `are_building_cards_visible() -> bool` | 设置、切换和读取底部建筑卡行显隐；不改变左上镜卡。 |
...
| `MirrorDefinition.get_formatted_inspection_description` / `_bbcode` | `() -> String` | 把基础描述和升级格式化为镜子卡悬停/选中镜子共用的两行纯文本或 BBCode。 |
| `CardCooldownSweep.gd` | `set_state(ready_ratio, blocked) -> void` | 绘制镜卡灰层与自上而下扫描线；不读取或修改任何美术资产。 |
| `RuntimeHud.gd` | `configure(interaction: RuntimeInteractionController, time_controller: GameTimeController, resource_manager: ResourceManager, building_manager: BuildingManager, mirror_manager: MirrorManager, slot_count: int = 6, stuff_editor_controller: Node = null) -> void` | 组合可建造卡槽、按箭塔/冰冻塔/导弹塔/镭射塔顺序注入只读图鉴，并连接交互、经济、时间、暂停与可选运行时元素编辑器。 |
| `RuntimeHud.gd` | `set_building_cards_visible(visible: bool) -> void` / `toggle_building_cards() -> void` / `are_building_cards_visible() -> bool` | 向 Main 暴露正式建筑卡显隐入口。 |
...
| `InspectionDisplayConfig.gd` | `format_semantic_description(fallback: String, headings: PackedStringArray, descriptions: PackedStringArray) -> String` | 把 Definition 提供的语义标题/正文输出为无标记纯文本。 |
| `InspectionDisplayConfig.gd` | `format_semantic_description_bbcode(fallback: String, headings: PackedStringArray, descriptions: PackedStringArray) -> String` | 依行序位输出绿/黄/红粗体标题和默认白色正文，并仅执行受支持标记。 |
