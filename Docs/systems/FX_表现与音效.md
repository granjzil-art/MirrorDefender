...
- 全部实时反射面与复制镜实体背板只在可见层 20；所有反射相机剔除层 20，复制镜和水面不会彼此递归采样，复制镜也不会把自身蓝色背板拍满反射纹理。
- 反射 Camera3D 复制主相机 Environment，但不复制最终视口 CameraAttributes；微缩 DOF 只由主视口执行一次，反射纹理不会被预先虚化。
...
- 反射相机当前不继承主相机的任何 CameraAttributes；若未来加入独立相机曝光参数，需要显式复制曝光字段，同时继续排除 DOF。
