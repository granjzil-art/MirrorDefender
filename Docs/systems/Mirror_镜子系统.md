...
> 状态：M5 复制镜已实现并通过 Godot 4.7.1 回归（2026-07-20）；反射镜属于 M6。
...
- 投影保留源内容的几何轮廓与主色，叠加青蓝色半透明、发光材质，确保一眼可辨认“实体/虚像”。
- 同格多项投影用很小的垂直层偏移避免完全重叠；偏移仅影响表现，不改变玩法坐标。
...
| `projection_alpha` | 0.46 | CopyMirrorDefinition | 投影透明度 |
| `projection_tint` | 青蓝 | CopyMirrorDefinition | 投影颜色叠加 |
| `projection_layer_offset` | 0.035 格 | CopyMirrorDefinition | 同格投影视觉层偏移 |
...
| `scripts/mirror/MirrorProjection.gd` | `MirrorProjection` / `Node3D` | 非占位虚像表现、屏障代理和地块效果代理。 |
...
| `tests/copy_mirror_test.gd` | `SceneTree` | 42 项双网格与联调回归。 |
...
- 初版投影使用程序化灰盒几何，不包含正式模型或动画资产。
- 最近源格仅沿镜面法线离散扫描，不支持任意角度射线穿格。
- 投影攻击忠实复制原攻击，允许因镜像空间无目标而打空。
- 程序化虚像按 `copy_kind` 提供灰盒轮廓；正式资产接入后应由 payload 的表现工厂替代，不改变覆盖层查询。
