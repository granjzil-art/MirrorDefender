...
> 状态：M5 复制镜与镜面/虚像表现优化已实现并通过 Godot 4.7.1 回归（2026-07-20）；会改变激光方向的反射镜仍属于 M6。
...
- 大石头投影：目标格加入动态寻路阻挡层；敌人在阻碍前一格触发既有换路逻辑。
...
- 建筑投影复制 `Building` 当前 `_visual_root` 的静态视觉快照；地块元素投影通过 `TileRenderer.create_tile_visual_snapshot()` 复用正常地块的地表、真实高度、侧壁、颜色、障碍与元素几何。
...
| `mirror_cost` | 120 | CopyMirrorDefinition | 放置消耗 |
| `mirror_refund` | 60 | CopyMirrorDefinition | 删除返还 |
| `mirror_height_ratio` | 1.20 格 | CopyMirrorDefinition | 镜框、实时镜面、标识和操作锚点共用的默认高度 |
| `projection_ignores_occupancy` | true | CopyMirrorDefinition | 投影是否忽略实体/投影占位并允许叠加 |
| `copy_chain_max` | 4 | CopyMirrorDefinition | 最大镜链深度 |
| `reflection_enabled` | true | CopyMirrorDefinition | 是否启用复制镜生效面的实时世界反射 |
| `reflection_resolution` | 256 | CopyMirrorDefinition | 正式镜面的水平渲染分辨率 |
| `reflection_preview_resolution` | 128 | CopyMirrorDefinition | 放置预览镜面的水平渲染分辨率 |
...
| `mirror_reflectivity` | 0.92 | CopyMirrorDefinition | 反射画面相对镜面底色的混合比例 |
...
CopyMirror
  ...
  └─ MirrorReflectionView（共享世界离轴视锥，只渲染生效面）
...
| `scripts/mirror/CopyMirrorDefinition.gd` | `CopyMirrorDefinition` / `Resource` | 经济、链深、占位开关与虚像表现参数。 |
...
- 反射相机通过近裁面和离轴视锥裁切镜后空间；极端贴面或近平行观察角可能出现边缘裁切，参数上限用于控制开销而非消除该几何极限。
- 为阻止递归渲染，反射相机不渲染所有镜子的反射表面；镜中仍能看到其它镜子的镜框和背板，但不会出现无限镜廊。
