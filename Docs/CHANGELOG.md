...
- 正式投影、镜子放置预览和建筑放置预览复用同一连线节点；预览实例复用时同步更新曲线序号与合法性，不参与碰撞、占位、伤害或寻路。
- 颜色、宽度、拱高、采样数、流速、流动密度、发光和端点高度均进入 `CopyMirrorDefinition`，并可在运行时战斗参数编辑器调整。
**影响面**：只增加复制关系的可视化，不改变复制来源、镜链递归、投影位置、伤害、占位或镜子经济。

## Tutorial / Wave Action · 四关逐波自动投放一级塔 — 2026-08-19
**模块**：Tutorial / Wave / Building / Runtime Editor / Runtime UI / Tests / Docs。
- 教程事件新增“波次结束自动建塔”配置，可指定普通塔、地图格和36档朝向；免费生成、固定一级、计入建筑上限，并且每个事件只尝试一次。
- 预设格不可用时按当前正方形网格距离寻找最近合法格，同距离按坐标稳定排序；全图无合法格或上限已满时失败，不覆盖玩家建筑、不绕过路径连通性。
- Level1–Level4 已重排初始塔与后续奖励，使每关最终塔数等于原建筑上限；奖励时强制暂停并显示获得塔弹窗，确认后在新塔脚下播放5秒提示圈，左侧塔图鉴同步标记各塔从第几波开始可用。
- 运行时教程编辑器支持选择塔型、朝向，并将鼠标移至地图格后按 `T` 采样目标格；专项回归覆盖四关资源、运行时事务、UI与回退边界。
**影响面**：修改四个正式关卡的初始塔/教程事件和教学反馈；不改变镜子陈列、敌人波次、路线、经济、塔数值或建筑上限。

## Mirror / Presentation · 二级镜子边框新增金光 — 2026-08-17
**模块**：Mirror / Presentation / Tests / Docs。
- 复制镜和反射镜升至二级时，原镜体边缘即时切换为金色发光材质；降回一级恢复普通边框，初始二级镜子加载后直接显示。
- 金光开关、颜色和发光强度进入 `MirrorDefinition` 可调参数；选中状态不会削弱金光，放置/迁移预览仍优先显示绿/红合法性颜色。
- Forward+ 近景验收加入镜面、金框和中央法线箭头的组合画面，升级专项扩展至 73 项。
**影响面**：只改变二级实体镜的边框表现，不修改等级数值、升级经济、镜面反射、法线箭头、碰撞或选取。

## Mirror / Presentation · 镜面中央新增法线方向箭头 — 2026-08-17
**模块**：Mirror / Presentation / Tests / Docs。
- 复制镜、反射镜及两种放置预览在镜体中央新增程序化短箭头，沿当前玩法生效面的法线向外显示；翻面和预览移动时同步更新。
- 箭头开关、颜色、长度、杆/箭头尺寸、离面距离与发光强度进入 `MirrorDefinition` 可调参数；默认使用橙红色无光照发光材质。
- 箭头不创建碰撞或拾取节点，并使用镜面反射相机排除的表现层，避免进入镜中画面。
**影响面**：只增加镜面方向提示，不修改复制来源、投射物反射、镜子放置、选中、经济或碰撞逻辑。

## Runtime UI / Economy · 选中塔与镜子新增降级操作 — 2026-08-16
**模块**：Runtime UI / Building / Mirror / Resource / Tests / Docs。
- 塔的选中面板改为左降级、右升级两项，去掉说明和拆除；降级使用新的向下箭头素材。
- 镜子保留上出售/右升级，左说明改为降级；1 级隐藏降级，满级隐藏升级，费用与退款数字继续使用统一金币黄。
- 塔每次降级返还升到当前级时的费用；镜子降级返还真实升级投入并同步减记可售价值，避免重复退款。降级后保持选中并立即刷新战斗参数、产出与镜像。
**影响面**：改变实体选中操作和升降级经济；建造、卡片悬停说明、键盘旋转/镜子翻面、镜子出售和底层建筑拆除 API 保持。

## Runtime UI · 新增左侧四塔只读图鉴 — 2026-08-16
**模块**：Runtime UI / Building Definitions / Tests / Docs。
- 左侧镜卡下方新增箭塔、冰冻塔、导弹塔、镭射塔四张纵向只读图鉴卡，卡面仅显示建筑图标和名称，不显示价格。
- 图鉴卡不可选中且不发送建造信号；悬停时在卡右侧显示与建筑卡/选中说明同源的自适应格式化文本。
- 图鉴默认常驻且独立于 F2；既有底部可建造卡的默认关闭、F2 切换和全部交互保持不变，F1 调试入口同步移位避让图鉴。
**影响面**：只增加局内塔信息图鉴与左侧 HUD 布局；不修改建造、费用、容量、选中、镜子或战斗逻辑。

## Balance / Level1 · 追加双盾士兵与精英法师 — 2026-08-16
**模块**：Level1 / Wave / Enemy Roster / Tests / Docs。
- 保留Level1原有十五波全部投放，从W8开始追加双盾士兵、W10开始追加精英法师，W10–W15交替双路混编。
- 共新增双盾士兵18只、精英法师8只；新增组均落在原有生成窗口内，不改变既有组、路径组合或原投放时长。
- Level1正式敌人扩充为七种，弓箭手仍不使用；新增35项专项配置回归。
**影响面**：只修改Level1敌人波次；Level2–Level4、敌人基础数值、经济倍率和完成奖励逻辑不变。

## Runtime UI · 镜卡迁至左上并新增 F2 建筑卡开关 — 2026-08-16
**模块**：Runtime UI / Input / Tests / Docs。
- 复制镜、反射镜从底部建筑卡组拆出，按 14px 间距常驻左上并排；悬停说明在上方空间不足时自动改到镜卡下方。
- 底部建筑卡进入关卡时默认关闭，新增物理 F2 InputMap 动作控制开/关；切换不会隐藏镜卡，也不取消当前选择或放置状态。
- 调整左上调试摘要和卡面模式按钮避让，并增加三档分辨率、F2 输入链、镜卡/建筑卡独立显隐及 Forward+ 双状态验收。
**影响面**：只改变卡槽布局和建筑卡可见性入口；卡片内容、费用、容量、建造、镜子放置与战斗逻辑不变。

## Runtime UI · 重启与返回标题增加暂停二次确认 — 2026-08-16
**模块**：Runtime UI / Level Flow / Time / Tests / Docs。
- 右侧按钮、暂停菜单及胜利/失败结果页的重启和返回入口统一改为屏幕中央确认层，按动作显示“将重启关卡，确认吗”或“将返回标题，确认吗”。
- 打开确认层立即暂停玩法且不提前发送高层请求；确认后沿用原重载/退关事务，取消则恢复打开前的运行或暂停状态与既有玩家倍率。
- 扩展 RuntimeHud 与 AppFlow 流程回归，覆盖运行中取消恢复 4x、结果页取消保持 0x、确认前零请求和确认后单次请求。
**影响面**：只改变重启/返回标题的触发保护和模态 UI；关卡重载、选关返回、胜负判定及倍率档位事实源不变。

## Wave / Economy / Balance · 每波完成固定奖励40金币 — 2026-08-16
**模块**：Wave / Resource / Economy / Balance / Tests / Docs。
- `WaveDefinition` 新增默认40的可编辑完成奖励；Level1–Level4现有60波全部填充为40，每关15波合计额外发放600金币。
- 当该波待生成组和存活敌人均清空时，`WaveManager` 在已有波次完成去重事务中通过 `ResourceManager` 入账；手动叠波各自结算，重复检查不多发。
- 奖励原因标记为 `wave_completion`，不受敌人掉落倍率影响；新增69项四关60波配置契约，手动叠波运行时回归扩展至81项。
**影响面**：每关固定增加600波次完成收益；不修改敌人击杀掉落、初始/被动经济、建筑费用、波次编排或HP成长。

## Runtime UI · 拉开右侧波次操作按钮间距 — 2026-08-15
**模块**：Runtime UI / Wave Controls / Tests / Docs。
- 释放下一波、快速重开、退出关卡三个右侧按钮的纵向间距从 8px 增加到 18px，并同步扩展居中布局高度。
- 新增间距死区与三档分辨率布局回归，确保按钮不越界、不与右上统计或底部卡槽重叠。
**影响面**：只调整右侧三按钮的间距和容器高度；按钮尺寸、功能、右边距与波次逻辑不变。
...
## Debug / Runtime UI · 调试工具进入关卡默认收起 — 2026-08-15
**模块**：Debug / Runtime UI / Lighting / Tests / Docs。
- 调试总开关的初始状态改为关闭；进入关卡时隐藏并停用控制台、运行时关卡/参数编辑、左上调试摘要、卡槽表现切换和灯光/树影/实树测试栏。
- 首次按 F1 完整呼出调试工具，后续 F1 继续循环显隐；已启用分类的挂起/恢复语义不变，正式镜头键 1/2 始终生效。
**影响面**：只改变调试工具的局内初始可见性和可用性；不修改调试分类保存状态、正式镜头功能或玩法输入。

## Building UI · 弹道预览改为固定粗细 — 2026-08-14
**模块**：Building / Runtime UI / Tests / Docs。
- 用单一 `projectile_preview_width` 取代“最小宽度 + 实际投射物宽度倍率”，选中和放置弹道不再随等级、投射物或激光束尺寸变粗变细。
- 增加回归断言，覆盖修改投射物宽度后预览仍保持固定线宽，以及持续激光共用相同预览宽度。
**影响面**：仅改变预览线网格；投射物模型、碰撞体、伤害宽度和射程不变。

## Mirror / Combat · 二级镜子攻击效果初版 — 2026-08-14
**模块**：Mirror / Combat / Projectile / Laser / Tests / Docs。
- 新增资源化 `MirrorAttackEffect` 与运行时 `AttackEffectPayload`，分离镜子等级效果、攻击状态、递归镜链继承和反射/命中分支预算，为后续新复制/反射效果留出扩展钩子。
- 二级复制镜复制箭塔时追加爆裂箭：母箭每次穿透命中都向水平八方发射 8 枚可配子箭；递归复制保留效果，子箭不会二次爆裂。
- 二级反射镜在正常反射方向的左右各 15° 生成完整攻击分支；标准/复制投射物、导弹、持续激光和脉冲镭射均保留反射后当前伤害、剩余路程/穿透/反射数与特殊效果快照。
**影响面**：保留复制镜伤害/穿透累计、反射镜原路径、原有反射修正和一级行为；新效果只在对应镜子达到二级时激活。
...
- 纯朝向模式完全跳过索敌和目标锁定，按冷却沿逻辑朝向自动发射方向弹，并隐藏不再适用的蓝色索敌范围。
- 运行时战斗数据编辑器增加“仅固定朝向”选项，建筑检视明确显示“不索敌”；复制塔继续同步方向攻击事件。
- 新增独立专项回归，覆盖枚举兼容、配置校验、有敌时仍不索敌、固定方向、冷却周期和索敌范围隔离。
**影响面**：不自动迁移任何正式资源；只有显式将某等级配置为 `FACING_ONLY` 才启用新行为。
...
## Economy / Balance · 掉落改为前十波1.00、后五波0.50 — 2026-08-13
**模块**：Wave / Economy / Balance / Tests / Docs。
- Level1–Level4 W1–W5 掉落倍率由1.50恢复为1.00，W6–W10继续1.00，W11–W15由0.60降为0.50；当前经济简化为前十波与后五波两段。
- 四关理论全关击杀收益更新为2590、3009、4218、4856，对比上一版分别减少421.9、496.8、706.0、791.4。
- 波次契约锁定W1–W10为1.00、W11–W15为0.50及四关新收益总量；数值文档保留1.50/1.00/0.60原配置与分项回退方式。
**影响面**：仅修改击杀经济倍率；不修改波次投放、HP成长、生成间隔、初始/被动收入或建筑/升级费用。

## Runtime UI / Wave · 手动释放波次退出选中时缓 — 2026-08-13
**模块**：Runtime UI / Wave / Selection / Time / Tests / Docs。
- 右侧“释放下一波”按钮成功释放波次后，通过独立玩家操作信号调用 RuntimeInteractionController 的统一取消选择事务。
- 当前选卡、已选建筑、已选镜子、世界选择和放置预览一并清除，战术慢放上下文随之结束；时间恢复到玩家此前记住的 1x/2x/4x 档位。
- 只有按钮成功释放波次才执行；调试命令释放波次、失败/禁用点击和用户“慢放开关”偏好不受影响。
**影响面**：只改变手动释放波次后的选择与时间上下文；不修改波次内容、生成时序、重叠释放、快进档位或战术慢放配置。

## Camera / Input · 运行时数字键仅保留正式镜头 1/2 — 2026-08-13
**模块**：Camera / InputMap / Lighting / Depth of Field / Debug / Tests / Docs。
- 数字键收敛为只有 `1/2` 切换两个正式游戏镜头，且不受 F1 调试开关影响；移除 `3–6` 镜头 InputMap、`6–9` 灯光切换和 `0` 景深对比键。
- 灯光方案继续通过无数字前缀的左上按钮切换；六槽镜头资源和关卡编辑器格式继续兼容，但槽位 3～6 不再拥有运行时键盘入口。
- 回归显式约束 InputMap、CameraPresetController 数字动作表及灯光/景深脚本，防止其它数字键功能回流。
**影响面**：只收敛运行时数字键所有权；不修改镜头 1/2 数据、切换插值、灯光方案、景深设置或六槽作者资源格式。
...
- 胜利画面复用结果模态与现有高层事务，只显示“重启关卡”和“返回标题”两个操作；重启深重载当前关卡，返回标题回到现役选关页且不退出程序。
- 回归覆盖 `0/5/15/15以上` 边界、星级文本、输入/时间锁、双按钮数量、同关重启及 AppFlow 返回标题完整流程。
**影响面**：新增单局即时胜利评价与胜利结果操作；不保存星级、不修改判胜条件、据点生命、关卡解锁或选关进度。

## Economy / Balance · 后五波掉落倍率收紧至0.60 — 2026-08-13
**模块**：Wave / Economy / Balance / Tests / Docs。
- Level1–Level4 的 W11–W15 敌人掉落倍率由 `0.75` 统一降为 `0.60`；W1–W5 `1.50`、W6–W10 `1.00` 保持不变。
- 调整后四关理论全关击杀收益分别为3011.9、3505.8、4924.0、5647.4，对比0.75方案分别减少263.1、344.7、492.0、600.6。
- 波次契约同步锁定后五波0.60倍率和四关调整后收益总量；数值记录保留0.75原配置和独立回退方式。
**影响面**：仅收紧四关W11–W15击杀经济；不修改前10波掉落、敌人投放、HP成长、初始/被动经济或建筑费用。

## Runtime UI / Time · 1x/2x/4x 循环倍率与状态色 — 2026-08-13
**模块**：Runtime UI / Time / Tests / Docs。
- 原 2x 按钮改为 `1x → 2x → 4x → 1x` 单按钮循环；当前档位分别使用浅绿、浅橙、浅红背景。
- 慢放按钮使用浅紫色，暂停/继续按钮使用浅灰色；悬停和按下保留同色系明暗反馈。
- 暂停与战术慢放继续高优先级覆盖实际时间，但会记住玩家所选 1x/2x/4x，解除后恢复；旧快速布尔接口继续兼容 2x。
- 回归覆盖三档循环、五类颜色、4x 的暂停/慢放恢复、重置回 1x 以及旧 2x 调用兼容。
**影响面**：新增 4x 和时间按钮状态色；不改变战术慢放触发条件、暂停语义、真实时间 UI 动画或玩法数值。

## Level / Wave / Economy / Balance · 前期投放减压与分段击杀收益 — 2026-08-12
**模块**：Level / Wave / Economy / Balance / Tests / Docs。
- Level1–Level3 保留W1–W5原敌人数量、兵种、路线和剧情节点，仅拉长普通组生成间隔，新调整间隔上限3s；W6–W15战斗投放不变。
- Level4 前五波改为 `1路→1路→2路→2路延迟增援→4路齐出`；四路同时投放由W3顺延到W5，W5改用四条短路，长路首次登场顺延到W6，每波敌人总数和兵种不变。
- 新增波次掉落倍率：W1–W5 `×1.50`、W6–W10 `×1.00`、W11–W15 `×0.75`；敌人出生时快照所属波次倍率，手动叠波/倒序击杀不串经济，调试与测试敌人保持1倍。
- 数值专项文档新增完整原配置、实施值、压力/收益变化、重载竞态踩坑、实战门槛和独立回退步骤。
**影响面**：四关W1–W5投放密度、Level4前期路线揭示、全15波击杀收益时点；不修改敌人数量、基础数值、W6–W15战斗编排、初始资源、被动经济、建筑/升级价格或HP成长公式。

## Debug / Runtime UI · F1 统一调试工具总开关 — 2026-08-12
**模块**：Debug / Runtime HUD / Runtime Editors / Lighting / Tests / Docs。
- 控制台和运行时参数编辑器取消原 F1/F2 独立显隐热键；左侧改为常驻“调试控制台 / 运行时关卡编辑 / 运行时参数编辑”三按钮组，参数窗口在非源码开发运行时保持可见但禁用。
- F1 改为统一控制调试按钮组、控制台、调试摘要、卡槽表现切换及左上灯光/树影/实树测试栏的显隐和可用性；已开启的路径等调试分类只被暂时挂起，重新开启后恢复原状态。
- 调试总开关关闭时会收起原生参数窗口并阻止隐藏的关卡编辑工具继续接收编辑输入；未保存的工作副本不被静默丢弃。
**影响面**：只收敛开发调试入口和输入边界；不修改灯光当前方案、树影/实树当前状态、关卡作者数据、战斗参数或正式玩法快捷键。

## Level / Wave / Balance · Level2–Level4 十五波关卡剧本 — 2026-08-12
**模块**：Level / Wave / Enemy / Balance / Tests / Docs。
- Level2、Level3、Level4 各扩展为15波，分别围绕等长镜像双路、102/22格极端长短路、四出生点八路线双据点设计独立节奏与策略考题。
- 双盾士兵在Level2第6波确定性亮相；精英法师在Level3第4波确定性亮相；正式四关只使用步兵、疾行者、飞行侦察兵、单盾、双盾、法师、泰坦七种敌人，弓箭手不再用于正式波次。
- Level2用左右职责反转和双泰坦镜像终局验证换防；Level3用到达时差和24秒延迟泰坦验证时间管理；Level4用逐步点亮四门、双城等HP压力和四泰坦顺时针终局验证全图统筹。
- 新增后三关波次专项契约并纳入全量测试入口，2361项检查覆盖45波数据、七怪白名单、弓箭手零引用、路线配对和关键策略节点。
**影响面**：Level2–Level4波次数量、组合、时序、整关时长、收益和难度体验；不修改敌人基础数值、HP成长公式、经济、路径几何或统一漏怪扣1规则。
...
## Level / Wave / Balance · Level1 九波扩展为十五波 — 2026-08-12
**模块**：Level / Wave / Enemy / Balance / Tests / Docs。
- Level1 正式波次从9波扩展为15波，按基础火力、双路切换、反射防护、空地混合、终局攻坚五个三波阶段重新编排。
- 保持 `base × min(1.1^(wave-1), 5)` 的敌人 HP 成长；总计501只敌人，第15波倍率约 3.797，标称 HP 投放强度从约 74 平滑增长至 341 HP/秒。
- 所有出怪组显式写入数量、路线、出生点、延迟、间隔和抖动；教学组0–5%、常规组10%、尾波最高15%，不再继承30%默认抖动。
- 新增 Level1 十五波专项契约并纳入全量测试入口；数值专项文档保留原九波完整回退快照、决策依据、风险和实战待验项。
**影响面**：Level1 波次数量、出怪组合、出怪时序、随机抖动、整关时长与可获得击杀收益；敌人基础定义、HP成长公式、路径几何和统一漏怪扣1不变。
