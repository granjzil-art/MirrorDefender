...
func _test_square_runtime_surface_and_renderer() -> void:
	...
	_expect(is_equal_approx(terrain.get_world_height(Vector3i(1, 1, 0)), 0.125), "first 1:2 ramp center samples quarter rise")
	_expect(is_equal_approx(terrain.get_world_height(Vector3i(2, 1, 0)), 0.375), "second 1:2 ramp center samples three-quarter rise")
	_expect(is_equal_approx(terrain.get_world_height(Vector3i(3, 1, 0)), 0.5), "two-layer flat exposes one layer of height")
	_expect(is_equal_approx(tile.get_world_height(Vector3i(2, 1, 0)), 0.375), "legacy TileManager height query delegates to TerrainManager")
	...
	_expect(is_equal_approx(terrain.sample_surface_height(Vector3i(2, 1, 0), high_edge), 0.5), "ramp high edge is continuous with high flat")
	...
	_expect(is_equal_approx(Vector3(hit.get("pos", Vector3.ZERO)).y, 0.125), "ramp picking returns the sloped surface Y")
...
func _test_terrain_model_instancing() -> void:
	...
	terrain_definition.ramp_1_to_2_model_asset = _make_model_asset(Vector3(1.0, 1.0, 1.0))
...
func _test_hex_ramp_sampling() -> void:
	...
	_expect(is_equal_approx(terrain.get_world_height(Vector3i.ZERO), 0.25), "hex 1:1 ramp center is half a layer high")
	var normal := terrain.get_surface_normal(Vector3i.ZERO)
	_expect(normal.y > 0.9 and not normal.is_equal_approx(Vector3.UP), "hex ramp exposes an uphill plane normal")
	fixture["host"].queue_free()
	await process_frame
...
func _test_legacy_snapshot_runtime() -> void:
	...
	_expect(is_equal_approx(terrain.get_layer_height(), 0.4), "legacy height_step becomes canonical layer height")
