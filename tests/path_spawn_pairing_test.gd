...
func _test_editor_creation_and_wave_binding() -> void:
	...
	_expect(added_path_overlays.size() == 1 and added_path_overlays[0] == second_path, "adding a path isolates the newly selected route")
	var path_select := panel.get("_path_select") as OptionButton
	path_select.select(0)
	panel.call("_on_path_selected", 0)
	var visible_overlays: Array = path_canvas.get("_overlay_paths")
	_expect(visible_overlays.size() == 1 and visible_overlays[0] == first_path, "path page shows only the selected path overlay")
	...
	_expect(visible_overlays.size() == 1 and visible_overlays[0] == second_path, "switching the path selection immediately switches the isolated overlay")
	path_select.select(0)
	panel.call("_on_path_selected", 0)
	...
	path_select.select(0)
	panel.call("_on_path_selected", 0)
	panel.call("_remove_selected_path")
	visible_overlays = path_canvas.get("_overlay_paths")
	_expect(level.paths.size() == 1 and visible_overlays.size() == 1 and visible_overlays[0] == second_path, "deleting the selected path isolates the remaining selected route")
	var empty_level := LevelResource.new()
	empty_level.grid_shape = GridManager.Shape.SQUARE
	empty_level.grid_size = Vector2i(2, 2)
	panel.call("_set_level", empty_level)
	_expect((path_canvas.get("_overlay_paths") as Array).is_empty(), "a level without paths leaves the path overlay empty")
