...
func _run() -> void:
	...
	rig._input(_mouse_button(MOUSE_BUTTON_RIGHT, false))
	_expect(drag_yaw < 0.0 and drag_pitch < 50.0, "right/up drag uses the inverted yaw and pitch directions")
	_expect(_cancel_requests == 0, "right drag above threshold never emits cancel")

	rig.apply_view_state(Vector3.ZERO, 0.0, 20.0, 16.0)
	rig._input(_mouse_button(MOUSE_BUTTON_RIGHT, true))
	rig._input(_mouse_motion(Vector2(0.0, -20.0)))
	rig._input(_mouse_button(MOUSE_BUTTON_RIGHT, false))
	_expect(is_equal_approx(rig.get_pitch_angle(), 18.0), "inverted upward mouse orbit clamps lowered pitch to 18 degrees")
	rig._set_pitch(80.0)
	rig._input(_mouse_button(MOUSE_BUTTON_RIGHT, true))
	rig._input(_mouse_motion(Vector2(0.0, 20.0)))
	rig._input(_mouse_button(MOUSE_BUTTON_RIGHT, false))
	_expect(is_equal_approx(rig.get_pitch_angle(), 82.0), "inverted downward mouse orbit clamps raised pitch to 82 degrees")
