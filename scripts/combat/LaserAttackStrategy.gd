...
func _tick_burst(
	...
	while _burst_elapsed >= interval:
		...
		var endpoint: Vector3 = path.get("endpoint", maximum_end)
		apply_endpoint_burst(building, combat_manager, endpoint, true)
		building.call(
			"notify_copy_attack",
			&"laser_burst",
			start,
...
static func trace_laser_path(
	...
	return ContinuousLaserPathScript.trace(
		...
		direction,
		maximum_distance,
		int(building.call("get_projectile_penetration_count")),
...
static func apply_continuous_hits(
	...
	for raw_hit in raw_hits:
		...
		var applied := target.take_damage_over_time(damage_per_second, duration)
		if notify_source:
...
static func apply_endpoint_burst(
	...
	var burst_damage: float = building.call("get_laser_burst_damage")
