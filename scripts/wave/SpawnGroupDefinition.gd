...
## Each gap is sampled independently from interval * (1 +/- interval_jitter_ratio).
## A zero ratio restores exact fixed-interval spawning.
@export_range(0.01, 1000.0, 0.01, "or_greater") var interval: float = 0.8
@export_range(0.0, 0.95, 0.01) var interval_jitter_ratio: float = 0.30
